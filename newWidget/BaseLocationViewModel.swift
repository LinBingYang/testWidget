...
typealias LocationSuccessInfo = ([String:AnyObject]) -> Void
...
class BaseLocationViewModel:NSObject {
    ...
    var locationSuccessInfo:LocationSuccessInfo?
    /// manager
...
//    }
    func userLocationInfo(locationSuccessInfo: @escaping LocationSuccessInfo,
        ...
        self.locationSuccessInfo = locationSuccessInfo
        self.locationFail = locationFail
...
extension BaseLocationViewModel{
    ...
    private func reverseGeocodeLocation(location:CLLocation){
        ...
        locationGeocoder.reverseGeocodeLocation(location){(placemark, error) in
            ...
            print("反地理编码-country：" + (placemark?.first?.country ?? ""))
            /// eg. CA
            ...
            let distist = placemark?.first?.subLocality ?? ""
            let country = placemark?.first?.country ?? ""
            let thoroughfare = placemark?.first?.thoroughfare ?? ""
            var dict:NSDictionary? = nil
            dict=["city":city,"distist":distist,"country":country,"thoroughfare":thoroughfare]
            /// 回调得到的位置信息
//            guard let locationPlacemark = placemark?.first else{return}
//            guard let locationSuccess = self.locationSuccess else{return}
            guard let dict1 = dict else{return}
            guard let locationSuccessInfo = self.locationSuccessInfo else{return}
//            locationSuccess(locationPlacemark)
            locationSuccessInfo(dict1 as! [String : AnyObject])
//            LocationSuccessInfo(dict)
            /// 地理编码位置，看能不能得到正确经纬度
